...
function CountryLists({ country }) {
  ...
  React.useEffect(()=>{
    countries?.map((ele)=>country === ele.name ? console.log("back " + ele.name + " " + ele.img): null)

    setBackImg( countries?.map((ele)=>country === ele.name ? ele.img : null))
  ...
  return (
    <div>
      <CountryIndia countryName={country} cityList={countries} count ={count} events ={events} backImg={backImg}/>
